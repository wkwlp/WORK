...
class LoveQuoteService:
    ...
    def get_quote(self):
        ...
        try:
            # 调用API获取随机情话数据
            hzbh,quote_data = self.love_quote_api.get_random_quote()

            # 处理获取到的情话数据
            content = self.handle_quote(hzbh=hzbh,quote=quote_data)
            if not content:
                self.logger.warning("获取情话中content为空,返回None")
                return None
            handler_content = content.strip()
            # 返回去除空白字符后的情话内容
            return hzbh,handler_content
    ...
    def handle_quote(self,hzbh,quote):
        """
        从API响应数据中提取情话内容。
        ...
        self.love_mysql.update_love_api(hzbh=hzbh,quote=content)
        return content
